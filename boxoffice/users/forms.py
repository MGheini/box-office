...
class OrganizerRegModelForm(forms.ModelForm):
	...
	organizer_last_name = forms.CharField(required=True, label='نام خانوادگی', widget=forms.TextInput(attrs={'placeholder': ' مثلا: پاشاخانلو'}))
	organizer_username = forms.CharField(required=True, label='نام کاربری', widget=forms.TextInput(attrs={'placeholder': ' مثلا: mozhpar'}))
	organizer_password = forms.CharField(required=True, label='رمز عبور', widget=forms.PasswordInput(attrs={'placeholder': ' هیچگاه گذرواژتان را فاش نکنید.'}))
	organizer_password2 = forms.CharField(required=True, label='تکرار رمز عبور', widget=forms.PasswordInput(attrs={'placeholder': ' تکرار گذرواژه'}))
	organizer_email = forms.EmailField(required=True, label='رایانامه', widget=forms.TextInput(attrs={'placeholder': ' مثلا: mozhpar@gisheh.ir'}))
